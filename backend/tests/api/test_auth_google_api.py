"""Google login + e2e bootstrap — Google verification is mocked."""
...
@pytest.fixture
def with_google_client_id(monkeypatch):
...
def test_google_login_returns_401_on_invalid_token(
    client: TestClient, monkeypatch, with_google_client_id
):
    ...
    resp = client.post("/auth/google-login", json={"token": "bad"})
    assert resp.status_code == 401


def test_google_login_returns_401_when_payload_missing_sub(
    client: TestClient, monkeypatch, with_google_client_id
):
    from app.api import auth
    ...
    resp = client.post("/auth/google-login", json={"token": "ok"})
    assert resp.status_code == 401


def test_google_login_creates_user_and_sets_session_cookie(
    client: TestClient, monkeypatch, with_google_client_id
):
    from app.api import auth
