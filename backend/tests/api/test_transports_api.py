...
# ---------- POST /transports/telegram/capture-user-id ---------------------------


def test_capture_hello_requires_token(authed_client: TestClient, stub_transports):
    resp = authed_client.post("/transports/telegram/capture-user-id")
    assert resp.status_code == 400
...
def test_capture_hello_returns_hint_when_no_match(
    ...
    body = authed_client.post("/transports/telegram/capture-user-id").json()
...
# ---------- POST /transports/telegram/send-message -------------------------------------


def test_send_message_requires_token(authed_client: TestClient, stub_transports):
    resp = authed_client.post("/transports/telegram/send-message", json={"text": "hi"})
    assert resp.status_code == 400
...
def test_transports_endpoints_require_auth(client: TestClient):
    ...
    assert client.post("/transports/telegram/capture-user-id").status_code == 401
    assert client.post("/transports/telegram/send-message", json={"text": "x"}).status_code == 401
