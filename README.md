...
## ENV
Для корректной работы приложения заполните env variables из соответствующих для frontend и backend `.env.example`.
Заметьте, что для работы потребуется OAuth 2.0 Client IDs by Google. Создайте приложение и заполните соответствующие env'ы с секретами от Google.

## Backend
Все команды описаные ниже запускаются из директории `backend`
### Локальный запуск
...
## Frontend
Все команды описаные ниже запускаются из директории `frontend`
### Локальный запуск
Для запуска frontend используем npm скрипты:
```bash
npm install
npm run dev
```
